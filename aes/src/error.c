...
static void aes_collect_call_stack(AES_ErrorDetails* err_details) {
    err_details->call_stack_len =
        CaptureStackBackTrace(1, AES_MAX_CALL_STACK_LENGTH, err_details->call_stack, NULL);
}
#else
static void aes_collect_call_stack(AES_ErrorDetails* err_details) {
...
static AES_StatusCode aes_make_error(AES_ErrorDetails* err_details, AES_StatusCode ec) {
    if (err_details == NULL)
        return ec;
...
AES_StatusCode aes_success(AES_ErrorDetails* err_details) {
...
AES_StatusCode aes_error_parse(AES_ErrorDetails* err_details, const char* src, const char* what) {
...
AES_StatusCode aes_error_invalid_pkcs7_padding(AES_ErrorDetails* err_details) {
