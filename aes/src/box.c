...
AES_StatusCode aes_box_init(AES_Box* box,
...
static AES_StatusCode aes_box_encrypt_block_ecb(AES_Box* box,
...
static AES_StatusCode aes_box_encrypt_block_cbc(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->xor_block(&xored_input, &box->iv, err_details)))
        return status;
...
static AES_StatusCode aes_box_encrypt_block_cfb(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->xor_block(output, input, err_details)))
        return status;
...
static AES_StatusCode aes_box_encrypt_block_ofb(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
...
static AES_StatusCode aes_box_encrypt_block_ctr(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->inc_block(&box->iv, err_details)))
        return status;

    return status;
}

typedef AES_StatusCode (*AES_BoxEncryptBlockInMode)(AES_Box*,
                                                    const AES_BoxBlock*,
                                                    AES_BoxBlock*,
                                                    AES_ErrorDetails*);
...
AES_StatusCode aes_box_encrypt_block(AES_Box* box,
                                     const AES_BoxBlock* input,
                                     AES_BoxBlock* output,
                                     AES_ErrorDetails* err_details) {
...
static AES_StatusCode aes_box_decrypt_block_ecb(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
...
static AES_StatusCode aes_box_decrypt_block_cbc(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->xor_block(output, &box->iv, err_details)))
        return status;
...
static AES_StatusCode aes_box_decrypt_block_cfb(AES_Box* box,
                                                const AES_BoxBlock* input,
                                                AES_BoxBlock* output,
                                                AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
...
AES_StatusCode aes_box_decrypt_block(AES_Box* box,
                                     const AES_BoxBlock* input,
                                     AES_BoxBlock* output,
                                     AES_ErrorDetails* err_details) {
...
static AES_StatusCode aes_box_get_encrypted_buffer_size(AES_Box* box,
                                                        size_t src_size,
                                                        size_t* dest_size,
                                                        size_t* padding_size,
                                                        AES_ErrorDetails* err_details) {
    ...
    switch (box->mode) {
        ...
        case AES_CBC: {
            ...
            if (aes_is_error(status = box->algorithm->get_block_size(&block_size, err_details)))
                return status;
...
static AES_StatusCode aes_box_encrypt_buffer_block(AES_Box* box,
                                                   const void* src,
                                                   void* dest,
                                                   AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->load_block(&plaintext, src, err_details)))
        return status;

    AES_BoxBlock ciphertext;

    if (aes_is_error(status = aes_box_encrypt_block(box, &plaintext, &ciphertext, err_details)))
        return status;

    if (aes_is_error(status = box->algorithm->store_block(dest, &ciphertext, err_details)))
        return status;
...
static AES_StatusCode aes_box_encrypt_buffer_partial_block_with_padding(
    ...
    if (aes_is_error(status = aes_box_encrypt_buffer_block(box, plaintext_buf, dest, err_details)))
        goto FREE_PLAINTEXT_BUF;
...
static AES_StatusCode aes_box_encrypt_buffer_partial_block(AES_Box* box,
                                                           const void* src,
                                                           size_t src_size,
                                                           void* dest,
                                                           AES_ErrorDetails* err_details) {
...
AES_StatusCode aes_box_encrypt_buffer(AES_Box* box,
                                      const void* src,
                                      size_t src_size,
                                      void* dest,
                                      size_t* dest_size,
    ...
    for (size_t i = 0; i < src_len; ++i) {
        if (aes_is_error(status = aes_box_encrypt_buffer_block(box, src, dest, err_details)))
            return status;
...
static AES_StatusCode aes_box_get_decrypted_buffer_size(AES_Box* box,
                                                        size_t src_size,
                                                        size_t* dest_size,
...
static AES_StatusCode aes_box_decrypt_buffer_block(AES_Box* box,
                                                   const void* src,
                                                   void* dest,
                                                   AES_ErrorDetails* err_details) {
    AES_StatusCode status = AES_SUCCESS;
    ...
    if (aes_is_error(status = box->algorithm->load_block(&ciphertext, src, err_details)))
        return status;

    AES_BoxBlock plaintext;

    if (aes_is_error(status = aes_box_decrypt_block(box, &ciphertext, &plaintext, err_details)))
        return status;

    if (aes_is_error(status = box->algorithm->store_block(dest, &plaintext, err_details)))
        return status;

    return status;
}

static AES_StatusCode aes_box_decrypt_buffer_partial_block(AES_Box* box,
                                                           const void* src,
                                                           size_t src_size,
                                                           void* dest,
                                                           AES_ErrorDetails* err_details) {
...
AES_StatusCode aes_box_decrypt_buffer(AES_Box* box,
                                      const void* src,
                                      size_t src_size,
                                      void* dest,
                                      size_t* dest_size,
    ...
    for (size_t i = 0; i < src_len; ++i) {
        if (aes_is_error(status = aes_box_decrypt_buffer_block(box, src, dest, err_details)))
            return status;
    ...
    if (max_padding_size == 0) {
        ...
        if (aes_is_error(status = aes_extract_padding_size(AES_PADDING_PKCS7,
...
AES_StatusCode aes_box_parse_block(AES_BoxBlock* dest,
                                   AES_Algorithm algorithm,
                                   const char* src,
                                   AES_ErrorDetails* err_details) {
    if (dest == NULL)
...
AES_StatusCode aes_box_parse_key(AES_BoxKey* dest,
                                 AES_Algorithm algorithm,
                                 const char* src,
                                 AES_ErrorDetails* err_details) {
    if (dest == NULL)
...
AES_StatusCode aes_box_format_block(AES_BoxBlockString* dest,
                                    AES_Algorithm algorithm,
...
AES_StatusCode aes_box_format_key(AES_BoxKeyString* dest,
                                  AES_Algorithm algorithm,
