...
; Copied from asm/aes128.asm

.code

aes128_encrypt_block_internal proc
    movdqa xmm0, [rcx]
    pxor xmm0, [rdx]
    aesenc xmm0, [rdx + 10h]
    aesenc xmm0, [rdx + 20h]
    ...
    aesenc xmm0, [rdx + 80h]
    aesenc xmm0, [rdx + 90h]
    aesenclast xmm0, [rdx + 0A0h]
    ret
aes128_encrypt_block_internal endp

aes128_decrypt_block_internal proc
    movdqa xmm0, [rcx]
    pxor xmm0, [rdx]
    aesdec xmm0, [rdx + 10h]
    aesdec xmm0, [rdx + 20h]
    ...
    aesdec xmm0, [rdx + 80h]
    aesdec xmm0, [rdx + 90h]
    aesdeclast xmm0, [rdx + 0A0h]
    ret
aes128_decrypt_block_internal endp

aes128_expand_key_internal proc
    ...
    movdqa [rdx], xmm0    ; sets w[0], w[1], w[2], w[3]
    add rdx, 10h          ; rdx = &w[4]
...
aes128_keygen_assist:
    ...
    movdqa [rdx], xmm0    ; w[i+7] = RotWord(SubWord(w[i+3]))^Rcon^w[i+3]^w[i+2]^w[i+1]^w[i]
                          ; w[i+6] = RotWord(SubWord(w[i+3]))^Rcon^w[i+2]^w[i+1]^w[i]
                          ; w[i+5] = RotWord(SubWord(w[i+3]))^Rcon^w[i+1]^w[i]
                          ; w[i+4] = RotWord(SubWord(w[i+3]))^Rcon^w[i]
    add rdx, 10h          ; rdx = &w[i+8]

    ret
aes128_expand_key_internal endp

aes128_derive_decryption_keys_internal proc
    movdqa xmm5, [rcx]
    movdqa xmm4, [rcx + 0A0h]
    movdqa [rdx], xmm4
    ...
    ret
aes128_derive_decryption_keys_internal endp
